...
elseif isUnstructuredFun && ~isUnstructuredAna
  ...
  % dastkari
  close all
  figure, plot3(functional.pos(:,1), functional.pos(:,2), functional.pos(:,3), '.')
  hold on, plot3(anatomical.pos(:,1), anatomical.pos(:,2), anatomical.pos(:,3), '.r') 
%   ll=1;
%   for ii=1:anatomical.dim(1)
%       for jj=1:anatomical.dim(2)
%           for kk=1:anatomical.dim(3)
%               tmp = anatomical.transform*[ii;jj;kk;1];
%               anat_pos(ll,:)=tmp(1:3);
%               ll=ll+1;
%           end
%       end
%   end
  ll=1;
  for kk=1:length(functional.pos)
      tmp = anatomical.transform*[functional.pos(kk,:)';1];
      func_pos(ll,:) = tmp(1:3);
      ll=ll+1;
  end
  figure, plot3(func_pos(:,1), func_pos(:,2), func_pos(:,3), '.')
  hold on, plot3(anatomical.pos(:,1), anatomical.pos(:,2), anatomical.pos(:,3), '.r')
  functional.pos = func_pos;
