function source_activation_mri3(cfg, mri,dat,pos)
...
f = figure('WindowScrollWheelFcn',@figScroll,'WindowButtonUpFcn',@ImageClickUp,'Name','Source Activity on MRI',...
    'pos',[1 1 400*layout(2) 400*layout(1)]);
...
%%
    function plotData()
        ...
        scatter(ax1, gr2(:),gr1(:), 30, kk(:), 's', 'filled');
        else 
      ...
      ax3.Tag='13';
        ...
        if useZoom
            ...
            scatter(ax1, gr1(:),rot90(gr2(:),2), 30, kk(:), 's', 'filled');
        else
     ...
     %   colorbar(ax2);
      ...
      ax3.Tag='23';
        ...
        if useZoom
            ...
            scatter(ax1, rot90(gr1(:),-1),gr2(:), 30, kk(:), 's', 'filled');
        else
